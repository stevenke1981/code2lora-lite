...
- `audit.json`：token 減量門檻的 pass/fail gate
- `session-audit.json`：context pack 加上 agent 實際打開檔案後的 pass/fail 估算
- `opened-files.txt`：由 `scripts/agent-open.ps1` 自動記錄的實際開檔清單
...
之後才視任務需要打開原始碼檔案。
wrapper 在未達 `-MinReduction` 時會 non-zero 失敗，讓 token 減量不是只有提示，而是可驗收的 gate。
任務結束時可用 `scripts/agent-session-audit.ps1`，將 raw repo baseline 與
`context.md` 加上 `.code2lora/agent-context/opened-files.txt` 裡記錄的實際開檔成本做比較。
讀取原始碼時使用 `scripts/agent-open.ps1`，可自動維護 opened-files log。
支援 MCP 的 Codex/OpenCode client 可把 `scripts/code2lora-mcp.ps1` 當 stdio server。
repo-local config 範例在 `mcp/codex.example.toml` 與 `mcp/opencode.example.jsonc`。
Windows 使用 `scripts/install-mcp-config.ps1`，Linux/macOS + PowerShell 7+
...
│   ├── agent-open.ps1              # 打開原始檔並記錄 session 使用量
│   ├── agent-session-audit.ps1     # 審計實際 session token 減量
